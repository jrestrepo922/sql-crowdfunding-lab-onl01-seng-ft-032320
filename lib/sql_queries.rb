...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT u.name, SUM(pl.amount) FROM users u
   JOIN pledges pl ON pl.user_id = u.id GROUP BY u.name ORDER BY SUM(pl.amount); "
end

def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  "SELECT pr.category, pl.amount FROM projects pr
   JOIN pledges pl ON pl.project_id = pr.id WHERE pr.category = 'music'"
end

def selects_the_category_name_and_the_sum_total_of_the_all_its_pledges_for_the_books_category
  "SELECT pr.category, SUM(pl.id) FROM projects pr
   JOIN pledges pl ON pl.project_id = pr.id WHERE pr.category = 'books' "
end
